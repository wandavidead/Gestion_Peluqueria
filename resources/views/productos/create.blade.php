...
<div class="col-md-8 offset-md-2 mt-5">
	<div class="card card-outline-secondary">
		...
		<div class="card-body">
			...
			@endif
			<form class="form" method="POST" action="{{ route('productos.store') }}" role="form" enctype="multipart/form-data">
				{{ csrf_field() }}
				<div class="form-group row">
					<label class="col-lg-3 col-form-label form-control-label"
						>@lang('Product denomination')</label
					>
					<div class="col-lg-9">
						<input
							type="text"
							name="denominacion_producto"
							id="denominacion_producto"
							class="form-control input-sm"
							placeholder="@lang('Product denomination')"
						/>
						@if ($errors->has('denominacion_producto'))
						<span class="text-danger"
							>{{ $errors->first('denominacion_producto') }}</span
						>
						@endif
					</div>
				</div>
				<div class="form-group row">
					<label class="col-lg-3 col-form-label form-control-label"
						>@lang('Brand of the product')</label
					>
					<div class="col-lg-9">
						<input
							type="text"
				...
				<div class="form-group row">
					<label class="col-lg-3 col-form-label form-control-label"
						>@lang('Price of the product')</label
					>
					<div class="col-lg-9">
						<input
							type="text"
					...
					<div class="col-lg-9">
						<select name="categoria_id" id="categoria_id" class="form-control select-sm">
							...
							<option value="{{ $categoria->id }}"
								>{{ $categoria->id }} - {{ $categoria->nombre_categoria }}</option
							>
							@endforeach
						</select>
				...
				<div class="form-group row">
					...
					<div class="col-lg-9">
						...
						>
							<option>@lang('Select the provider')</option>
							@foreach($proveedores as $proveedor)
							<option value="{{ $proveedor->id }}"
								>{{ $proveedor->id }} - {{ $proveedor->denominacion_social }}</option
							>
							@endforeach
						</select>
